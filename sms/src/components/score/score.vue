<template>
  <div>
    <el-card class="cardStyle" v-loading="loading">
      ...
      <el-button @click="sendEmailMethod" type="warning" :disabled="dataTable.length <= 0" size="small" style="margin-bottom: 15px" v-if="userInfo.level !== 2">发送邮件</el-button>
      <!--过滤框-->
...
<script>
  ...
  export default {
    ...
    methods : {
      ...
      sendEmailMethod () {
        if (this.selection.length === 0) {
          this.$message.warning('请先选择要发送邮件的学生');
          return;
        }
        this.$prompt('请输入学生邮箱地址', '发送成绩邮件', {
          ...
          const studentId = this.selection[0].no;
          this.axiosHelper.post('/api/sms/score/sendEmail', {
            studentId: studentId,
            email: value
          }).then(response => {
