<template>
  <el-card style="margin: 10px;padding: 10px 10px 0 10px;height: 80vh;">
    <el-row>
      ...
      <el-button type="warning" size="small" @click="generateRandom" v-if="this.userInfo.level === 0"
                 :disabled="!form.profession || !form.grade || !form.year">随机生成</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      generateRandom () {
        this.$confirm('确定要生成随机课程吗？这会覆盖现有课程表', '提示', {
          ...
          this.axiosHelper.post('/api/sms/timetable/generateRandom', null, {
            params: obj
          }).then(() => {
            this.$message.success('随机课程生成成功');
            this.gradeChange(); // 刷新课程表
          }).catch(() => {
